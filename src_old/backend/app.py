...
def _apply_support_file_logic(raw, masking_paths=None, diary_paths=None, start_stop_paths=None):
    ...
    def apply_interval(start, stop, source):
        ...
        try:
            ...
            elif hasattr(raw, "mask_fraction"):
                summary["notes"].append(f"{source}: raw object exposes masking info but not add_mask_period in this install.")
            else:
                summary["notes"].append(f"{source}: interval parsed but this raw object does not expose add_mask_period.")
        except Exception as exc:
            summary["notes"].append(f"{source}: could not apply interval ({exc}).")

    for path in masking_paths or []:
        df = _read_support_table(path)
        start_col = _find_column(df, ["start", "start_time", "startTime", "onset", "begin"])
        stop_col = _find_column(df, ["stop", "end", "end_time", "stopTime", "offset"])
