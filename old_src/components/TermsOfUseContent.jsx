    ...
    <div style={{ display: "grid", gap: 12, textAlign: "left" }}>
      {!compact && (
        <div style={{ border: "1px solid #bfdbfe", borderRadius: 14, padding: 15, background: "#eff6ff", color: "#1e3a8a", lineHeight: 1.6 }}>
          ActiLab is hosted by the Ontario Brain Institute (OBI), with development supported through the Centre for Analytics. By using the tool, you agree to the conditions below and to any applicable institutional, research-ethics, privacy, and data-governance requirements.
        </div>
      )}
      ...
      <TermSection title="Temporary processing">
        Uploaded recording files, support files, job data, and temporary processing data are retained only as needed to operate the service and are automatically deleted no later than 30 days after upload. Some temporary files may be deleted sooner after processing or when a job completes. Download any outputs that you need to retain.
      </TermSection>

      <TermSection title="Technical context and feedback">
        Feedback may include non-raw technical information such as filenames, file sizes, selected settings, processing stages, errors, request IDs, and summary results. A contact email address is required so the Centre for Analytics team can follow up. Feedback, the email address, and attached technical context are retained for a maximum of 30 days from submission and then automatically deleted. A minimal notification email containing the submitter email, filename, workflow step, category, and feedback ID may be sent to the designated project contact; the feedback message and detailed diagnostic context are not included in that notification. Do not include participant identifiers, raw measurements, or other sensitive information in filenames or feedback text.
      </TermSection>
