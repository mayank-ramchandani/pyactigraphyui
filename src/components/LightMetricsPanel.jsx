...
export const LIGHT_METRIC_DEFINITIONS = {
  ...
  extremum_max: {
    ...
    summary: "Maximum light exposure timing and value.",
    description:
  ...
  return (
    <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 20, padding: 20 }}>
      ...
      <p style={{ color: "#64748b", marginTop: 0, marginBottom: 16 }}>
        Select light metrics in the same grouped workflow as activity and sleep metrics. Results will be generated on the next page with the rest of the analysis outputs.
      </p>
      ...
      <div style={{ border: "1px solid #dbeafe", borderRadius: 14, background: "#eff6ff", color: "#1e3a8a", padding: 14, marginBottom: 20, fontSize: 14, lineHeight: 1.55 }}>
        <div style={{ fontWeight: 800, marginBottom: 8 }}>Common light settings</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10 }}>
          <label>
            <div style={{ fontWeight: 600, marginBottom: 5 }}>Light channel</div>
            <select value={settings.channel} onChange={(e) => updateSetting("channel", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #93c5fd", background: "white" }}>
              <option value="">Auto/default channel</option>
              {channels.map((channel) => <option key={channel} value={channel}>{channel}</option>)}
            </select>
          </label>
          <label>
            <div style={{ fontWeight: 600, marginBottom: 5 }}>Threshold (lux)</div>
            <input value={settings.thresholdLux} onChange={(e) => updateSetting("thresholdLux", e.target.value)} placeholder="e.g. 10, 100, 500" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #93c5fd" }} />
          </label>
          <label>
            <div style={{ fontWeight: 600, marginBottom: 5 }}>Aggregation</div>
            <select value={settings.agg} onChange={(e) => updateSetting("agg", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #93c5fd", background: "white" }}>
              {["mean", "median", "sum", "std", "min", "max"].map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
          </label>
          <label>
            <div style={{ fontWeight: 600, marginBottom: 5 }}>Start time</div>
            <input value={settings.startTime} onChange={(e) => updateSetting("startTime", e.target.value)} placeholder="HH:MM:SS" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #93c5fd" }} />
          </label>
          <label>
            <div style={{ fontWeight: 600, marginBottom: 5 }}>Stop time</div>
            <input value={settings.stopTime} onChange={(e) => updateSetting("stopTime", e.target.value)} placeholder="HH:MM:SS" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #93c5fd" }} />
          </label>
          <label>
            <div style={{ fontWeight: 600, marginBottom: 5 }}>TAT output format</div>
            <select value={settings.outputFormat} onChange={(e) => updateSetting("outputFormat", e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #93c5fd", background: "white" }}>
      ...
      {LIGHT_METRIC_GROUPS.map((group) => {
        ...
        return (
          <div key={group.id} style={{ marginBottom: 16, border: "1px solid #e2e8f0", borderRadius: 16, overflow: "hidden", background: "#f8fafc" }}>
      ...
      <div style={{ marginTop: 18, border: "1px solid #e2e8f0", borderRadius: 16, padding: 16, background: "#f8fafc" }}>
        <div style={{ fontWeight: 800, marginBottom: 10 }}>Advanced light metric settings</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10 }}>
          <label>
            <div style={{ fontWeight: 600, marginBottom: 5 }}>Summary bins</div>
            <input value={settings.bins} onChange={(e) => updateSetting("bins", e.target.value)} placeholder="24h" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #cbd5e1" }} />
          </label>
          <label>
          ...
          <label>
            ...
            <input value={settings.lmxLength} onChange={(e) => updateSetting("lmxLength", e.target.value)} placeholder="5h or 10h" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #cbd5e1" }} />
          </label>
