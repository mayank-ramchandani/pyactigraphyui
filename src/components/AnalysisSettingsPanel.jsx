  ...
  return (
    <div style={{ border: "1px solid #dbeafe", borderRadius: 16, padding: 16, background: "#f8fbff", marginBottom: 20 }}>
      ...
      <div style={{ color: "#475569", fontSize: 13, lineHeight: 1.55, marginBottom: 16 }}>
        Use the data-quality settings below, or customize them for your study protocol or sensitivity analysis. These settings are applied during final analysis and are reported in exported provenance.
      </div>
      ...
      <div style={{ border: "1px solid #dcfce7", borderRadius: 16, padding: 16, background: "#f0fdf4", marginBottom: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 12 }}>
          <div style={{ background: "white", border: "1px solid #bbf7d0", borderRadius: 12, padding: 12 }}>
            ...
            <div style={{ color: "#475569", fontSize: 13, marginTop: 4 }}>Minimum analyzable recording time; customizable below.</div>
          </div>
          <div style={{ background: "white", border: "1px solid #bbf7d0", borderRadius: 12, padding: 12 }}>
            <div style={{ fontWeight: 800 }}>2 consecutive valid windows</div>
            <div style={{ color: "#475569", fontSize: 13, marginTop: 4 }}>Minimum uninterrupted run of valid quality windows.</div>
          </div>
          <div style={{ background: "white", border: "1px solid #bbf7d0", borderRadius: 12, padding: 12 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div className="two-option-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
          <label style={{ display: "block", border: resolved.validDayWindowMode === "calendar_day" ? "2px solid #2563eb" : "1px solid #cbd5e1", borderRadius: 14, padding: 13, cursor: "pointer", background: resolved.validDayWindowMode === "calendar_day" ? "#eff6ff" : "white" }}>
            ...
            <span style={{ marginLeft: 8, fontWeight: 800 }}>Calendar day</span>
      ...
      <label style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: 13, borderRadius: 12, border: "1px solid #dbeafe", background: "#eff6ff" }}>
        ...
        <span>
          <span style={{ display: "block", fontWeight: 700, color: "#1e3a8a" }}>Customize data-quality thresholds</span>
          <span style={{ display: "block", marginTop: 3, color: "#475569", fontSize: 13, lineHeight: 1.5 }}>Leave this off to use 16 hours, 2 consecutive valid windows, and 80% sleep-window coverage. Enable it when your protocol or sensitivity analysis requires different criteria.</span>
        </span>
      </label>
      ...
      {resolved.customizeDataQualityThresholds ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 14, marginTop: 16, border: "1px solid #e2e8f0", borderRadius: 16, padding: 16, background: "white" }}>
          <NumberField label="Minimum valid hours per quality window" value={resolved.minimumValidHoursPerDay} onChange={(value) => update({ minimumValidHoursPerDay: value })} min="1" max="24" step="0.5" help={`Allowed range: 1–24 hours. A ${modeLabel} below this amount is invalid for day/window-based summaries.`} />
          <NumberField label="Minimum consecutive valid windows" value={resolved.minimumValidDaysForRhythm} onChange={(value) => update({ minimumValidDaysForRhythm: value })} min="1" max="365" step="1" help="Valid quality windows must form one uninterrupted run; separated valid windows do not satisfy this rule." />
          <NumberField label="Minimum sleep-window coverage" value={resolved.minimumSleepWindowCoverage} onChange={(value) => update({ minimumSleepWindowCoverage: value })} min="0" max="1" step="0.05" help="Enter a proportion from 0 to 1. For example, 0.8 means at least 80% of expected epochs must remain scorable." info="This threshold is calculated separately for each sleep window. Expected epochs are compared with recorded and scorable epochs after gaps, non-wear, start/stop truncation, and masks are applied. Windows below the threshold are reported as unavailable rather than imputed." />
        </div>
      ) : (
        <div style={{ marginTop: 14, color: "#166534", fontSize: 13, fontWeight: 800 }}>
          Thresholds active: 16 h/{modeLabel} · 2 consecutive valid windows · 80% sleep-window coverage
        </div>
      )}
    </div>
  );
